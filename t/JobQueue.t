...

{
    ...
    my $i = $d->find_item_index($data, 'blah.guru');
    ...
    cmp_deeply($data->{removed_domains}, \@res, 'returned and stored removal is the same');
    push(@res, 'corrupt');
    ok(scalar(@{$data->{removed_domains}}) == 1, 'modifying returned value will not corrupt data');
}
...

{
    ...
    ok(!exists $data->{removed_domains}, "don't create key if domain can't be found");
